...
loadPyodide().then((x) => {
  ...
  Init();
  try { self.postMessage({ type: 'ready' }); } catch (e) { }
});
...
self.onmessage = function (event) {
  ...
  if (data && data.type === 'ping') {
    const initialized = !!self.run_predicate;
    try { self.postMessage({ type: 'pong', initialized }); } catch (e) { }
    return;
  }
  if (data.type == 'run_predicate') {
    ...
    if (!self.run_predicate) {
      ...
      result.set('error_message', 'Worker not initialized');
    ...
    let result = self.run_predicate(program, predicate).toJs();
    result.set('type', 'run_predicate')
    result.set('hide_error', data.hide_error);
    result.set('program', program);
    result.set('predicate', predicate);
